...
import os
import pandas as pd
from dataloader_nan import ExCustomDataset_fMRI, data_split, count_parameters_in_MB
...
def main():
    ...
    log_filename = "/home_data/home/wangyb12023/result/fMRI/GAT_output_{}.log".format(
    ...
    dataset = ExCustomDataset_fMRI(temp=temp, df=df, transforms=False)
    seed = 123
    ...
    dataset_val = ExCustomDataset_fMRI(temp=temp, df=df, transforms=False)
    ...
    model = Models.GATNET(500, 500, 256, 128, 8)
    model = model.cuda()
    ...
    with open('/home_data/home/wangyb12023/result/fMRI/results_2.txt', 'w') as f:
        ...
        for epoch in tqdm(range(150), file=f):
            ...
            for i, batch in enumerate(tqdm(train_queue)):
                    fmri, labels =batch[0].cuda(), batch[1]
                    optimizer.zero_grad()
                    # print(fmri.size())
                    out_feature, outputs = model(fmri)
            ...
            with torch.no_grad():
                ...
                if accuracy > 60 and sens > 0.6 and spec > 0.6 and auc >= auc_baseline:
                    ...
                    torch.save(state, f'/home_data/home/wangyb12023/result/fMRI/GATs/qualified_model_{len(qualified)}_GAT_1_1e-2.pth')
...
# 在绘图之前，添加一个正确的epoch范围
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss_GAT0.0001_1e-2_0.4sparse_adj')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Train and Validation Accuracy_GAT0.0001_1e-2_0.4sparse_adj')
    plt.legend()
