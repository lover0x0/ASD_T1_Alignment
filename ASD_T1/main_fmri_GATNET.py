...
def main():
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=8e-1)
        ...
        for epoch in tqdm(range(150), file=f):
            ...
            with torch.no_grad():
                ...
                if accuracy > 60 and sens > 0.6 and spec > 0.6 and auc >= auc_baseline:
                    ...
                    torch.save(state, f'/home_data/home/wangyb12023/result/fMRI/qualified_model_{len(qualified)}_GAT_1.pth')
...
# 在绘图之前，添加一个正确的epoch范围
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss_GAT0.001')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Train and Validation Accuracy_GAT0.001')
    plt.legend()
